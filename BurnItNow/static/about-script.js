document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSlides() {
        let slides = document.getElementsByClassName("member-section");
        // Hide all slides
...
document.addEventListener("DOMContentLoaded", function() {
    const memberImages = document.querySelectorAll(".member-images");

    memberImages.forEach((image, index) => {
        if (index % 2 === 0) {
            image.classList.add("hover-down");
        } else {
            image.classList.add("hover-up");
        }
    });
});
